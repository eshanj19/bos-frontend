...
import { Card, CardContent, Menu, MenuItem, Button, Grid } from "@material-ui/core";
          ...
          <CardContent>
            ...
            <div>
              <div>
                ...
                {zeroEdgeNodes.length > 0 ? this.renderZeroEdgeNodes() : "-- No members --"}
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <Fragment key={item.key}>
          <Grid container alignItems='center'>
            ...
            <Grid item xs={2}>
              ...
              >
                <PersonAddIcon /> <span style={{marginLeft:'5px'}}>Set Parent</span>
              </Button>
            </Grid>
          </Grid>
          <Menu
          ...
          >
            {flatttenedHierarchyStructure.map(node => {
              return (
                <MenuItem
                  key={node.key}
                  onClick={() => {
                    this.setParentForNode(item.key, node.key);
                    this.setState({ menuAnchorEl: null });
                  }}
                >
                  {node.label}
            ...
            <MenuItem
              onClick={() => {
                this.setParentForNode(item.key, null);
                this.setState({ menuAnchorEl: null });
              }}
            >
              --Empty--
